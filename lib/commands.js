...
async function cmdWeather(chatId) {
  return sendMessage(chatId, await getWeather());
}

async function cmdAir(chatId) {
  return sendMessage(chatId, await getAirQuality());
}
...
async function cmdExpenses(chatId) {
  return sendMessage(chatId, formatTodayExpenses(await getTodayExpenses()));
}

async function cmdMonth(chatId) {
  return sendMessage(chatId, formatMonthExpenses(await getMonthExpenses()));
}
...
async function cmdAsk(chatId, question) {
  if (!question) {
    return sendMessage(chatId, "🩺 *Медичний консультант*\n\nПросто напиши питання або:\n/ask протокол лікування грижі L5-S1");
  }
  await sendMessage(chatId, "🩺 Аналізую...");
  const answer = await askMedical(question);
  return sendLong(chatId, answer);
}

async function cmdProtocols(chatId, args) {
  if (!args) {
    return sendMessage(chatId, "📋 *Пошук протоколів*\n\nФормат: /protocols діагноз\n\nПриклади:\n/protocols остеоартроз колінного суглобу\n/protocols ревматоїдний артрит\n/protocols цукровий діабет 2 типу\n/protocols гіпертонічна хвороба\n/protocols ГЕРХ");
  }
  await sendMessage(chatId, "📋 Шукаю протоколи...");
  const answer = await askProtocol(args);
  return sendLong(chatId, answer);
}

async function cmdCases(chatId, args) {
  if (!args) {
    return sendMessage(chatId, "📚 *Клінічні випадки*\n\nФормат: /cases тема\n\nПриклади:\n/cases грижа L5-S1 з радикулопатією\n/cases гонартроз 3 ступеня\n/cases подагричний артрит\n/cases компресійний перелом хребця\n/cases синдром зап'ясткового каналу");
  }
  await sendMessage(chatId, "📚 Створюю клінічний випадок...");
  const answer = await askClinicalCase(args);
  return sendLong(chatId, answer);
}

async function cmdNews(chatId, args) {
  await sendMessage(chatId, "📰 Збираю медичні новини...");
  const answer = await askMedicalNews(args);
  return sendLong(chatId, answer);
}
...
/sleep — сон | /heart — кардіо
/week — тижневий | /trends 30
...
*Погода:*
/weather — Шимкент | /air — повітря

*Фінанси (₸):*
/spend 5000 обід — витрата
/expenses — сьогодні | /month — місяць

*Медицина:*
🩺 Просто напиши питання — AI консультант
📸🧪 Фото/PDF аналізів — інтерпретація
/protocols діагноз — протоколи
/cases тема — клінічні випадки
/news тема — новини медицини

/briefing — повний звіт дня`;

const HELP = `📋 *Усі команди:*

📸 Фото — COROS або аналізи (авто)
📄 PDF — аналізи
💬 Текст — медичний AI
